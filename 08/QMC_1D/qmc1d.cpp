...
{
	return pow(val,4) - 5*pow(val,2)/2;
}
...
{
	return  (exp(-pow(v-m,2)/(2*s)) + exp(-pow(v+m,2)/(2*s)));
	//return exp(-0.5*v*v);
	//return 1;
}
...
{
	return (exp(-pow(v-m,2)/(2*s))*(pow(v-m,2)-s)/(s*s)  +  exp(-pow(v+m,2)/(2*s))*(pow(v+m,2)-s)/(s*s))/pow(2*sqrt(M_PI*s)*(1+exp(-m*m/s)),.5);
