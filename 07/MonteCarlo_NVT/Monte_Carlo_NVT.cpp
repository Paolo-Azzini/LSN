...
{ 
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      ...
      //print_istant_values("istant_values_solid.dat");
      //print_istant_values("istant_values_liquid.dat");
      print_istant_values("istant_values_gas.dat");
...
void Averages(int iblk) //Print results for current block
{
  ...
  if(iblk == 1){
    Epot.open("output.epot.0",ios::trunc);
    Pres.open("output.pres.0",ios::trunc);
    Gofr.open("output.gofr.0",ios::trunc);
...
//g(r)
    for(int i=igofr; i<n_props; i++){
      ...
      Gofr << setw(wd) << iblk <<  setw(wd) << r << setw(wd) << gdir << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
      if(iblk == nblk){
...
void print_istant_values(string filename){
	ofstream print;
	print.open(filename,ios::app);
	print << walker[iw]/(double)npart + ptail << " " << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
	print.close();
}
